# Agent 全栈深度解析 —— 内容架构 V2.0

## 核心理念：洋葱模型
从最外层的江湖故事开始，每剥一层就深入一个技术抽象层级，直到内核 JMP，再回到应用层。

```
Layer 0: 牛家村江湖（故事入口） ← 浅出
Layer 1: LLM 前世今生（模型层）
...
### Layer 0: 🎭 牛家村江湖（故事入口／比喻层）
> "从故事到代码" 的桥梁

| 场景 | 江湖故事 | 技术映射 |
|------|---------|---------|
| 牛家村密室训练 | 郭靖学武四阶段 | Pre-training → SFT → RLHF → FT |
| 对战梅超风 | 临敌出招全流程 | Runtime Tick → JMP → Tool Call |
| 襄阳大战 | 五绝协同作战 | Multi-Agent Fork → Workflow → PDCA |
| 华山论剑 | 暗号路由 vs 比武 | Auto-Routing → Skill dlopen |
| 桃花岛通信 | 飞鸽传书 | Channel → Message Queue |
| 密室墙谱 | 武功图谱检索 | Vector DB → RAG |
| 洪七公令旗 | 总指挥调度 | Orchestrator → fork() |

**功能设计**：
- 🃏 **人物卡牌**：点击人物查看技术对应，可翻转卡片
- 🗺️ **场景地图**：交互式场景导航，点场景跳转技术实现
- 📊 **完整映射表**：所有 technical ↔ jianghu 对应关系
- 📖 **剧情回放**：Workflow 映射为连续剧情
